...
rdaFitPCA
# gamma = 0.8 and lambda = 0.8.

accuracy <- c()

for (i in 1:5){
  cat(i)
  d <- createDataPartition(cancer_df$Relapse, p = 0.8, list = FALSE)
  # select training sample
  
  train <- cancer_df[d,]
  test  <- cancer_df[-d,]
  
  pre <- preProcess(train[,1:ncol(cancer_df) - 1], method = c("pca"), thresh = 0.9)
  
  rdaFitPCA <- train(Relapse ~ ., 
                       method = "rda",
                       tuneGrid = data.frame(gamma = 0.8, lambda = 0.8),
                       metric = "Kappa",
                       data = predict(pre, train),
                       trControl = trainControl(method = "none"),
                       preProcess = c("center", "scale"),
                       prior = c(0.5, 0.5))
  rdaPred = predict(rdaFitPCA, predict(pre,test))
